...
document.getElementById("openBankingAppButton").addEventListener("click", isIOS() ? showAppListPopup : openApp);
...
function isIOS() {
	return navigator.userAgent.match(/iPhone|iPod|iPad/) || (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
}
...
function closeAppListPopup() {
	var appListIframe = document.getElementById('appListIframe');
	if (appListIframe) {
		document.body.removeChild(appListIframe);
	}
}
...
function showAppListPopup() {
	if (parent.getAppList().length == 0) {
		...
		appListIframe = document.createElement('iframe');
		appListIframe.setAttribute("class", "more-about-iframe");
		appListIframe.setAttribute("id", "appListIframe");
		appListIframe.setAttribute("dt", Date.now());
		appListIframe.src = libUrl + "/html/app-list.html?url=" + appManifestUrl + "&requestType=" + requestType;
		document.body.appendChild(appListIframe);
	}
}


function openIOSApp(btn) {
	sendEvent("com.zapp.button.openapp.click", 0, btn);
}
