...
//			zapppopup.setHasAppCookie(cookieManagementUrl);
	...
	readJSONFile = function(file) {
		...
		readAppManifestFile = function(file) {
		if (file == "undefined" || file == null || file.length <= 0) {
			return;
		}
		if (zapppopup.appList.length > 0) {
			return;
		}
		var request = new XMLHttpRequest();
		request.onload = function() {
			...
			parent.getPayloadWithSignatureData(file, request.responseText.trim(), true).then(function(response) {
				...
				if (response !== false) {
					zapppopup.appList = response.apps;
					console.log(zapppopup.appList);
				}
			});
	...
	getAppList = function(shuffled) {
		if (shuffled) {
			appListShuffled = randomise(zapppopup.appList);
			return appListShuffled;
		} else {
			return zapppopup.appList;
		}
	}
