			...
			zapppopup,
			{
				...
				_init : function() {
					...
					if (typeof zapppopup.customMode != "undefined") {
						...
						self.addJsFile(zapppopup.libUrl + "/js/jsrsasign-latest-all-min.js");
						self.addJsFile(zapppopup.libUrl + "/js/appmanifest.js");
					}
				...
				_invokeApp : function(btn) {
					...
					var getUrl = function() {
						if (typeof zapppopup.requestType != "undefined") {
							if (zapppopup.requestType == "RequestToLinkAndPay") {
								url = "zapp-r4://" + self.secureToken+ "?apiName=RequestToPay";
							} else if (zapppopup.requestType == "RequestToPay") {
								url = "zapp://" + self.secureToken;
							} else {
								url = "zapp-r4://" + self.secureToken+ "?apiName=RequestToLink";
							}
						} else  {
							url = "zapp://" + self.secureToken;
						}
					...
					var getUrlForIOS = function() {
						if (typeof zapppopup.requestType != "undefined") {
							if (zapppopup.requestType == "RequestToLinkAndPay") {
								url = btn.data.appURLScheme + "://" + self.secureToken+ "?apiName=RequestToPay";
							} else if (zapppopup.requestType == "RequestToPay") {
								url = btn.data.appURLScheme + "://" + self.secureToken;
							} else {
								url = btn.data.appURLScheme + "://" + self.secureToken+ "?apiName=RequestToLink";
							}
						} else  {
							url = btn.data.appURLScheme + "://" + self.secureToken;
						}
					...
					var isInstalledCallback = function(isInstalled) {
						...
						zapppopup._popup.sendMessage(btn,
				...
				_invokeAppWithResponse : function(btn, response) {
					...
					if (typeof zapppopup.requestType != "undefined") {
						if (zapppopup.requestType == "RequestToLinkAndPay") {
							url = "zapp-r4://" + this.secureToken+ "?apiName=RequestToPay";
						} else if (zapppopup.requestType == "RequestToPay") {
	...
	zapppopup.popup = function(id) {
		...
		if (typeof zapppopup.customMode != "undefined") {
			brnIframe.src = zapppopup.libUrl + "/html/popup.html?libUrl="
					...
					+ "&appManifestUrl=" + zapppopup.options.appManifestURL
					+ "&requestType=" + zapppopup.requestType;
			...
			brnIframe.src = zapp.libUrl + "/html/popup.html?libUrl="
					...
					+ "&appManifestUrl=" + zapp.options.appManifestURL
					+ "&requestType=" + zapppopup.requestType;
			if (window.focus) {
				brnIframe.focus();
				}
